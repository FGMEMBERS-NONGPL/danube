...
# ===== plus coordinates for Danube Runabout 41.1 =====

# aircraft specific section:
var hatch_specs = {
	z_opening_ft: 6.0,
	z_floor_ft: -2.2,	#  z-axis offset for walker exit from aircraft origin
			#  generally floor z-axis converted to ft.
	rear_hatch_loc: nil,
	out_locations: func (loc) {
			if (loc == 0) {		# exit but not by hatch
				var new_coord = xy2LatLonZ(-11.4,-8.0);	# neutral coordinates outside front port quarter
			} elsif (loc == 1) {
				var new_coord = xy2LatLonZ(getprop("sim/model/danube/crew/walker/x-offset-m"),-3.5);
			} elsif (loc == 2) {
				var new_coord = xy2LatLonZ(getprop("sim/model/danube/crew/walker/x-offset-m"),3.5);
			} else {
...
var main_loop = func {
	...
	if (c_view == 0 and getprop("sim/walker/walking-momentum")) {
		# inside aircraft
		danube.walk_about_cabin(0.1, walk_heading);
		moved = 1;
	}
	if (getprop("sim/walker/outside")) {
		...
		if (elapsed_fall_sec > 0.75) {
			if (abs(getprop("sim/walker/altitude-ft") - getprop("position/altitude-ft") + hatch_specs.z_opening_ft) < 7) {
				...
				if (getprop("sim/model/danube/doors/door[0]/position-norm") > 0.73) {
					var door0_ll = xy2LatLonZ(-10.35,-1.87);
				...
				if (getprop("sim/model/danube/doors/door[1]/position-norm") > 0.73) {
					var door1_ll = xy2LatLonZ(-10.35,1.87);
...
var walker_model = {
	add:	func {
			if (getprop("sim/model/danube/crew/walker/visible")) {
				...
				setprop ("models/model/path", "Aircraft/danube/Models/walker.xml");
...
setlistener("sim/current-view/heading-offset-deg", func(n) {
	...
	if (c_view == 0) {
		...
		setprop("sim/model/danube/crew/walker/head-offset-deg" , head_v);
	} elsif (c_view == view.indexof("Walk View")) {
		var head_v = n.getValue();
		setprop("sim/walker/model-heading-deg" , 360 - head_v);
	}
...
var get_in = func (loc) {
	...
	var c_view = getprop("sim/current-view/view-number");
	# the following section is aircraft specific for locations of entry hatches and doors
	var c_pos = getprop("sim/model/danube/crew/cockpit-position");
	var c_head_deg = getprop("orientation/heading-deg");
	if (c_view > 0) {
		if (loc == 0) {	# find open hatch
			if (getprop("sim/model/danube/doors/door[0]/position-norm") > 0.2) {
				loc = 1;
				c_pos = 9;
			} elsif (getprop("sim/model/danube/doors/door[1]/position-norm") > 0.2) {
				loc = 2;
				c_pos = 9;
			} # else no door found
		}
		var new_walker_h = normheading(getprop("sim/current-view/heading-offset-deg") + c_head_deg);
		if (loc >= 1) { # change location to open hatch
			var new_walker_y = (loc == 1 ? -2.03 : (loc == 2 ? 2.03 : 0));
			if (c_pos == 9) { # pressed [o] jumps inside to different cockpit location then when left
				new_walker_h = (loc == 1 ? 276 : 84);
			}
			c_pos = 1;
			setprop("sim/model/danube/crew/cockpit-position", c_pos);
			var new_walker_x = -10.35;
		} else { # remember previous location
			var new_walker_x = danube.cockpit_locations[c_pos].x;
			var new_walker_y = danube.cockpit_locations[c_pos].y;
			new_walker_h = danube.cockpit_locations[c_pos].h;
		}
		var new_walker_z = danube.cockpit_locations[c_pos].z;
		var new_walker_p = danube.cockpit_locations[c_pos].p;
		var new_walker_fov = danube.cockpit_locations[c_pos].fov;
		setprop("sim/model/danube/crew/walker/x-offset-m", new_walker_x);
		setprop("sim/model/danube/crew/walker/y-offset-m", new_walker_y);
		# end aircraft specific
...
var init_common = func {
	...
	setlistener("sim/model/danube/crew/walker/visible", func(n) {
		if (n.getValue()) {
			if (getprop("sim/walker/outside")) {
				walker_model.add();
			}
